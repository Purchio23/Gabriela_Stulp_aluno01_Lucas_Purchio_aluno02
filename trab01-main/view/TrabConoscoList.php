...
include "../controller/TrabConoscoController.php";
  ...
  if(!empty($_GET['id'])){
    $TrabConosco->deletar($_GET['id']);
    header("location: TrabConoscoList.php");
  }
  if(!empty($_POST)){
    $load = $TrabConosco->pesquisar($_POST);

  }else{
    $load = $TrabConosco->carregar();
...
<br>
  <br>
    ...
    <div class="container">
    <div class="row">
    <h1>Lista de currículos Recebidos</h1>
    <form action="TrabConoscoList.php" method="post">
  ...
  <div class="col-6">
      ...
      <a class="btn btn-primary"href="TrabConoscoForm.php"><i class="fa-solid fa-plus"></i> Cadastrar</a>
  </div>
  ...
  <thead>
    ...
    foreach($load as $item){
      echo"<tr>
            ...
            <td>$item->CPF</td>
            <td>$item->funcao</td>
            <td><a href='./TrabConoscoForm.php?id=$item->id'><i class='fa-solid fa-pen-to-square'></i></a></td>
            <td><a href='./TrabConoscoList.php?id=$item->id'
